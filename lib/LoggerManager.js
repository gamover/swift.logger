...
LoggerManager.prototype.addLogger = function addLogger (logger)
{
    ...
    if (!(logger instanceof Logger))
        throw new $swiftErrors.TypeError('не удалось добавить логгер в менеджер логгеров. Недопустимый тип логгера (ожидается: "Logger", принято: "' + typeof logger + '")');

    loggerName = logger.getName();
    loggerSetName = logger.setName;

    if (loggerName in this._loggers)
        throw new $swiftErrors.ValueError('не удалось добавить логгер в менеджер логгеров. Логгер с именем "' + loggerName + '" уже существует');
    //
    ...
    logger.setName = function (name)
    {
        ...
        if (typeof name !== 'string')
            throw new $swiftErrors.TypeError('не удалось изменить имя логгеру "' + loggerName + '". Недопустимый тип имени (ожидается: "string", принято: "' + typeof name + '")');
        if (!name.length)
            throw new $swiftErrors.ValueError('не удалось изменить имя логгеру "' + loggerName + '". Пустое значение имени');
...
LoggerManager.prototype.removeLogger = function removeLogger (loggerName)
{
    ...
    if (typeof loggerName !== 'string')
        throw new $swiftErrors.TypeError('не удалось удалить логгер из менеджера логгеров. Недопустимый тип имени логгера (ожидается: "string", принято: "' + typeof loggerName + '")');
    if (!loggerName.length)
        throw new $swiftErrors.ValueError('не удалось удалить логгер из менеджера логгеров. Пустое значение имени логгера');
    //
    // удаление логгера
    //
