...
Logger.prototype.setPathToLog = function setPathToLog (pathToLogFile)
{
    ...
    //
    if (this._isRun)
        throw new $swiftErrors.SystemError('Логгер уже запущен.');
    if (typeof pathToLogFile !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип пути к лог-файлу (ожидается: "string", принято: "' + typeof pathToLogFile + '").');
    if (!pathToLogFile.length)
        throw new $swiftErrors.ValueError('Недопустимое значение пути к лог-файлу.');
    //
    // задание пути к лог-файлу
    //
...
Logger.prototype.setEncoding = function setEncoding (encoding)
{
    ...
    if (typeof encoding !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип кодировки (ожидается: "string", принято: "' + typeof encoding + '").');
    if (!encoding.length)
        throw new $swiftErrors.ValueError('Недопустимое значение кодировки.');
    //
    // задание кодировки
    //
...
Logger.prototype.run = function run (cb)
{
    ...
    if (this._isRun)
    {
        cb(new $swiftErrors.SystemError('Логгер уже запущен.'));
        return this;
    }
    if (this._pathToLogFile === null)
    {
        cb(new $swiftErrors.SystemError('Не задан путь к лог-файлу.'));
        return this;
    }
    ...
    {
        error = new $swiftErrors.SystemError('Ошибка запуска логгера (' + err.message + ').').setInfo(err);
    }
...
Logger.prototype.log = function log (text, cb)
{
    ...
    function log()
    {
        self._wStream.write(text + '\r\n', function (err)
        {
            if (err)
            {
                cb(new $swiftErrors.SystemError('Ошибка логирования (' + err.message + ').').setInfo(err));
                return;
            }
