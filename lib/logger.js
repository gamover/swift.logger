...
Logger.prototype.setName = function setName (name)
{
    ...
    if (typeof name !== 'string')
        throw new $swiftErrors.TypeError('не удалось изменить имя логгеру "' + this._name + '". Недопустимый тип имени логгера (ожидается: "string", принято: "' + typeof name + '")');
    if (!name.length)
        throw new $swiftErrors.ValueError('не удалось изменить имя логгеру "' + this._name + '". пустое значение имени логгера');
    //
    // задание имени логгера
    //
    this._name = name;
...
Logger.prototype.setPathToLog = function setPathToLog (pathToLogFile)
{
    ...
    //
    if (this._isRun)
        throw new $swiftErrors.SystemError('не удалось задать путь к лог-файлу логгеру "' + this._name + '". Логгер уже запущен');
    if (typeof pathToLogFile !== 'string')
        throw new $swiftErrors.TypeError('не удалось задать путь к лог-файлу логгеру "' + this._name + '". Недопустимый тип пути к лог-файлу (ожидается: "string", принято: "' + typeof pathToLogFile + '")');
    if (!pathToLogFile.length)
        throw new $swiftErrors.ValueError('не удалось задать путь к лог-файлу логгеру "' + this._name + '". Пустое значение пути к лог-файлу');
    //
    // задание пути к лог-файлу
    //
...
Logger.prototype.setEncoding = function setEncoding (encoding)
{
    ...
    //
    if (this._isRun)
        throw new $swiftErrors.SystemError('не удалось задать кодировку логгеру "' + this._name + '". Логгер уже запущен');
    if (typeof encoding !== 'string')
        throw new $swiftErrors.TypeError('не удалось задать кодировку логгеру "' + this._name + '". Недопустимый тип кодировки (ожидается: "string", принято: "' + typeof encoding + '")');
    if (!encoding.length)
        throw new $swiftErrors.ValueError('не удалось задать кодировку логгеру "' + this._name + '". Пустое значение кодировки');
    //
    // задание кодировки
    //
...
Logger.prototype.run = function run (cb)
{
    if (this._isRun)
    {
        cb(new $swiftErrors.SystemError('не удалось зпустить логгер "' + this._name + '". Логгер уже запущен'));
        return this;
    }
    if (this._pathToLogFile === null)
    {
        cb(new $swiftErrors.SystemError('не удалось зпустить логгер "' + this._name + '". Не задан путь к лог-файлу'));
        return this;
    }
    ...
    {
        cb(new $swiftErrors.SystemError('не удалось зпустить логгер "' + this._name + '". Возникла системная ошибка: ' + err.message).setInfo(err));
        return this;
    }
...
Logger.prototype.log = function log (text, cb)
{
    ...
    if (!this._isRun)
    {
        cb(new $swiftErrors.SystemError('не удалось произвести логирование логгером "' + this._name + '". Логгер не запущен'));
        return this;
    }
    ...
    function log()
    {
        self._wStream.write(text + '\r\n', function (err)
        {
            if (err)
            {
                cb(new $swiftErrors.SystemError('не удалось произвести логирование логгером "' + self._name + '". Возникла системная ошибка: ' + err.message).setInfo(err));
                return;
            }
